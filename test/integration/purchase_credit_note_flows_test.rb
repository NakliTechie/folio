...
class PurchaseCreditNoteFlowsTest < ActionDispatch::IntegrationTest
  setup do
    @org = Onboarding::SignUp.call(
      email: "purchase-credit-flow@folio.invalid",
      password: "correct-horse-battery",
      org_name: "Purchase Credit Flow"
    )
  ...
  test "owner records reviews and posts a supplier credit in the browser" do
    get new_purchase_credit_note_path, params: { purchase_bill_id: @bill.id }
    assert_response :success
    assert_select "h1", "Credit PB/26-27/00001"
    assert_select "input[name='purchase_credit_note[external_reference]']"
    assert_select "input[max='2.0']"

    assert_difference -> { Document.where(tenant_id: @org.tenant.id, doc_type: "PC").count }, 1 do
      post purchase_credit_notes_path, params: { purchase_credit_note: note_params }
    end
    note = Document.where(tenant_id: @org.tenant.id, doc_type: "PC").order(:id).last
    assert_redirected_to purchase_credit_note_path(note, tenant_id: @org.tenant.id)

    follow_redirect!
    assert_response :success
    assert_select "h1", "Draft supplier credit"
    assert_select "th", text: /IGST input-tax reduction/
    assert_select "td", text: /INR 29\.50/
    assert_select "button", text: "Post supplier credit"

    post post_purchase_credit_note_path(note)
    assert_redirected_to purchase_credit_note_path(note, tenant_id: @org.tenant.id)
    assert_equal "posted", note.reload.state
    assert_equal "PC/26-27/00001", note.document_number
    assert_equal 3, Entry.find(note.posted_entry_id).entry_lines.count

    get purchase_credit_notes_path
    assert_response :success
    assert_select "td", text: /PC\/26-27\/00001/
    assert_select "td", text: /V-CN-001/
    assert_select "td", text: /Acme Vendor/
  end

  test "operator creates and posts supplier credits through the JSON API" do
    operator = invite_user("purchase-credit-operator@folio.invalid", "operator")
    sign_out
    sign_in_as(operator)

    post "/api/v1/purchase_credit_notes", params: api_note_params
    assert_response :created
    json = JSON.parse(response.body).fetch("purchase_credit_note")
    assert_equal({ "igst" => 450 }, json.fetch("tax_breakdown"))
    assert_equal "V-CN-001", json.fetch("supplier_credit_note_number")
    assert_equal @bill.id, json.fetch("source_purchase_bill_id")

    post "/api/v1/purchase_credit_notes/#{json.fetch("id")}/post"
    assert_response :success
    posted = JSON.parse(response.body).fetch("purchase_credit_note")
    assert_equal "PC/26-27/00001", posted.fetch("document_number")
    assert_equal "operator", RoleTemplate.find(Entry.find_by!(document_id: json.fetch("id")).role_template_id).code
  end

  test "viewer may inspect supplier credits but cannot create them and tenant access is isolated" do
    note = PurchaseCreditNotes::BuildDraft.call(tenant: @org.tenant, **api_note_params)
    viewer = invite_user("purchase-credit-viewer@folio.invalid", "viewer")
    sign_out
    sign_in_as(viewer)

    get purchase_credit_notes_path
    assert_response :success
    get purchase_credit_note_path(note)
    assert_response :success
    get new_purchase_credit_note_path, params: { purchase_bill_id: @bill.id }
    assert_redirected_to root_path(tenant_id: @org.tenant.id)

    assert_no_difference "Document.count" do
      post "/api/v1/purchase_credit_notes", params: api_note_params.merge(external_reference: "V-CN-002")
    end
    assert_response :forbidden

    other = Onboarding::SignUp.call(
      email: "purchase-credit-other@folio.invalid",
      password: "correct-horse-battery",
      org_name: "Purchase Credit Other"
    )
    sign_out
    sign_in_as(other.user)
    get "/api/v1/purchase_credit_notes/#{note.id}"
