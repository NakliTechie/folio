...
module Api
  module V1
    class PurchaseDebitNotesController < BaseController
      ...
      def index
        render json: {
          purchase_debit_notes: document_scope.order(document_date: :desc).map { |note| document_json(note) }
        }
      end

      def show
        render json: { purchase_debit_note: document_json(@document) }
      end

      def create
        note = PurchaseDebitNotes::BuildDraft.call(
        ...
        render json: { purchase_debit_note: document_json(note) }, status: :created
      end
      ...
      def lines_params
        ...
        unless raw_lines.is_a?(Array)
          raise PurchaseDebitNotes::InvalidDebitNote, "lines must be an array"
        end

        raw_lines.map do |line|
          unless line.respond_to?(:permit)
            raise PurchaseDebitNotes::InvalidDebitNote, "each line must be an object"
          end
      ...
      def document_json(note)
        {
          ...
          supplier_debit_note_number: note.external_reference,
          source_purchase_bill_id: note.debit_note_for_document_id,
          ...
          lines: note.document_lines.map do |line|
            {
              ...
              source_purchase_bill_line_id: line.debited_document_line_id,
