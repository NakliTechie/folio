...
class SettlementReallocation < ApplicationRecord
  ...
  validates :tenant_id, :target_entry_line_id, :target_source_event_id, :target_ledger_id, :target_line_no,
  ...
  def target_item
    EntryLine.find_by(
      tenant_id: tenant_id, source_event_id: target_source_event_id,
      ledger_id: target_ledger_id, line_no: target_line_no
    )
  end
end
